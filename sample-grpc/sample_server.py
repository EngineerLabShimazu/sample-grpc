...
class SampleServiceServicer(sample_pb2_grpc.SampleServiceServicer):
    ...
    def HelloServer(self, request_iterator, context):
        for new_msg in request_iterator:
            reply_msgs = []
            print(
                'Receive new message! [name: {}, mgs: {}]'.format(new_msg.name,
                                                                  new_msg.msg))
            reply_msgs.append(sample_pb2.ReplyMessage(
                reply_msg='{} {}'.format(new_msg.msg, new_msg.name)))
            reply_msgs.append(
                sample_pb2.ReplyMessage(reply_msg='Nice to meet you!!!'))
...
def serve():
    ...
    sample_pb2_grpc.add_SampleServiceServicer_to_server(SampleServiceServicer(),
                                                        server)
